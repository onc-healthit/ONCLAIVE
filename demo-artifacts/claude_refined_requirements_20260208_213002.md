...
I have carefully analyzed all the provided FHIR Implementation Guide text chunks. After filtering out informational content, mapping tables, example instances, and purely definitional artifacts, here are the testable requirements:
...
**Context**: The qualification status must use codes from the QualificationStatusVS value set with required binding strength, meaning codes must come from this value set only.
**Conformance**: SHALL
**Actor**: Health Plan API Actor
**Sub-Requirement(s)**: NA
**Conditionality**: True
